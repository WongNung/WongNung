...
class BookmarkModelTest(TestCase):
    ...
    def setUp(self):
        ...
        self.fandom2 = Fandom.objects.create(name="DC Fans")
        # create bookmark
    ...
    def test_get_bookmark_independent(self):
        ...
        self.assertListEqual(
            expected_user2_fandom_bookmark_set, list(
                get_bookmark_set(ct=self.fandom_content_type, owner=self.user2)))
        expected_user3_fandom_bookmark_set = [self.bmk_fandom2]
        self.assertListEqual(
            expected_user3_fandom_bookmark_set, list(
                get_bookmark_set(ct=self.fandom_content_type, owner=self.user3)))
        self.assertListEqual(expected_user2_fandom_bookmark_set, list(
                get_bookmark_set(ct=self.fandom_content_type, owner=self.user2)))
        self.assertListEqual(expected_user3_fandom_bookmark_set, list(
                get_bookmark_set(ct=self.fandom_content_type, owner=self.user3)))
    ...
    def test_delete_bookmark(self):
        ...
        self.assertListEqual(expected_user1_bookmark_set, list(
                Bookmark.objects.filter(owner=self.user1)))

        delete_bookmark(
            self.fandom_content_type, owner=self.user3, obj_id=self.fandom2.id)
        self.assertListEqual(
    ...
    def test_delete_bookmark_independently(self):
        ...
        delete_bookmark(
            self.review_content_type, owner=self.user2, obj_id=self.review1.id)
        expected_user1_bookmark_set = [self.bmk_film1, self.bmk_film2, self.bmk_review1]
        self.assertListEqual(expected_user1_bookmark_set, list(
            Bookmark.objects.filter(owner=self.user1)))
